...
import React, {ComponentType, Element} from 'react';
...
export default class SwipeItem extends React.Component<Props, States> {
  ...
  _getSwipePositionDestinationValueX(panDistanceX: number): number {
    ...
    if (
      panSide === 'right' &&
      containerOffset > leftButtonTriggerPosition * buttonTriggerPercent
    ) {
    ...
    if (
      panSide === 'left' &&
      containerOffset < rightButtonTriggerPosition * buttonTriggerPercent
    ) {
